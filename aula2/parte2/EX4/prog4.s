...
# c:		$v0
	...
	.globl main
main:
	addiu $sp, $sp, -36		# preserve values in stack
	sw $ra, 0($sp)			#
	sw $s0, 4($sp)			#
	sw $s1, 8($sp)			#
	sw $s2, 12($sp)			#
...
if2:						#
	...
	li $s6, 1				# 		mult = 1;	
endif2:						#	}
	beq $v0, 'S', if3		#	if (c == 'S' || c == 's') {
	bne $v0, 's', endif3	#	
if3:						#
	li $s7, 'S'				#		counter = 'S'
endif3:						#	}
	beq $v0, 'R', if4		#	if (c == 'R' || c == 'r') {
	bne $v0, 'r', endif4	#	
if4:						#
	li $s7, 'R'				#		counter = 'S'
endif4:						#	}
	bne $s7, 'R', endif5	#	if (counter == 'R') {
	addi $s0, $s0, 1		#		cnt1++
	div $t1, $s3, $s4		#		// $t0 = frq1 / frq5
	rem $t0, $s0, $t1		#		// $t0 = cnt1%(frq1/frq5)
	bne $t0, 0, endif6		#		if (cnt1%(frq1/frq5) == 0) {
	addi $s1, $s1, 1		#			cnt5++
endif6:						#		}
	div $t1, $s3, $s5		#		// $t0 = frq1 / frq10
	rem $t0, $s0, $t1		#		// $t0 = cnt1%(frq1/frq10)
	bne $t0, 0, endif7		#		if (cnt1%(frq1/frq10) == 0) {
	addi $s2, $s2, 1		#			cnt10++
endif7:						#		}
endif5:						#	}
